...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple BrushNet training script.")
    ...
    parser.add_argument("--train_data_dir", type=str, default='/projects/yonglzha_proj/Our_sketch_textInp/PartialSketchNet/data/CUB-200-2011-test-tar/test_data.tar', help="Path to the tar file.")
    parser.add_argument("--save_dir", type=str,
                        default='/projects/yonglzha_proj/Our_sketch_textInp/PartialSketchNet/data/CUB-200-2011-test-tar/test_baseline_dataset_basedon_testdatatar',
...
class MyWebDataset():
    def __init__(self, resolution, random_mask, mask_area_range, dilate_len, blur_len, save_dir, canny_edge=None):
        ...
        with tarfile.open(tar_path, 'r') as archive:
            ...
            for member in members:
                if member.isfile() and member.name.endswith(".image"):
                    ...
                    self.s_masks = {}
                    # 1. 处理 caption 文本描述信息
                    caption_file = archive.extractfile(f"{class_img_name}.caption")
                    caption_text = random.choice(caption_file.read().decode('utf-8').splitlines())  # 将二进制数据解码为字符串 and splitlines() 将 caption 分割为多行文本

                    # 从 class_img_name 中提取 ':' 和 '#' 之间的字符串 for template text
                    if ':' in class_img_name and '#' in class_img_name:
                        start = class_img_name.index(':') + 1
                        end = class_img_name.index('#')
                        name = class_img_name[start:end]
                    caption_text = "A photo of the " + name.replace('_',' ').lower() + " bird."
                    ...
                    # 3. 处理 bounding box 信息,{list:4}
                    bbox_file = archive.extractfile(f"{class_img_name}.bbox")
                    bbox = json.loads(bbox_file.read().decode('utf-8'))  # 将 bounding box 字符串解析为字典或列表格式

                    # 4. 处理 segmentation 掩码, ndarray(h,w) nearly 255 represents object segmentation
                    seg_file = archive.extractfile(f"{class_img_name}.seg_mask")
                    seg_mask = cv2.imdecode(np.frombuffer(seg_file.read(), np.uint8), cv2.IMREAD_GRAYSCALE) # 解码为灰度图
...
def main(args):
    if args.seed is not None:
        set_seed(args.seed)
    dataset_instance = MyWebDataset(resolution=args.resolution, random_mask=args.random_mask,
